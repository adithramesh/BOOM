...
exports.getShopPage=(async(req,res)=>{
    ...
    const categoriesNotBlockedId=categories.map(category=>category._id)
      ...
      const categoriesWithCounts = await Promise.all(categories.map(async category => {
        const productCount = await Product.countDocuments({ category: category._id, isBlocked: false });
        return {
            name: category.name,
            count: productCount
        };
    ...
    const products=await Product.find({category:{$in:categoriesNotBlockedId},isBlocked:false}).sort(sortOptions);
...
exports.getProductDetailsPage=(async(req,res)=>{
    try{
        ...
        const productDetails=await Product.findById(productId).populate('category').exec()
...
exports.updateCartQuantity=async(req,res)=>{
    ...
    try {
      ...
      const item = cart.items.find(item => item._id.toString() === productId.toString() && item.size === size);
    ...
    try {
        ...
        console.log("items after forEach:",items)
         ...
         for (const item of items) {
            await Product.findOneAndUpdate({_id: item.product, "sizes.size": item.size},
...
exports.getOrders = async (req, res) => {
    try {
        ...
        const orders = await Order.find({ user: userId }).sort({ createdAt: -1 }).populate('items.product').exec();
...
exports.postResetPassword=async(req,res)=>{
    try{
        ...
        if(user.isBlocked==false && await bcrypt.compare(existingPassword,user.password)){
...
exports.postOtp=(async(req,res)=>{
    ...
    try{
        ...
        if(enteredOtp==otpInDb && expiry.getTime()>=Date.now()){
            ...
            }else if(req.session.otp){      //for forgot password otp
